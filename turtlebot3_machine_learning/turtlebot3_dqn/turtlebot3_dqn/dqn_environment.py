...
class RLEnvironment(Node):
    ...
    def make_environment_callback(self, request, response):
        ...
        future.add_done_callback(self._make_environment_done_cb)
        return response
    ...
    def _make_environment_done_cb(self, future):
        try:
            ...
            self.get_logger().error(f'initialize environment failed: {e}')
    ...
    def call_task_succeed(self):
        ...
        future.add_done_callback(self._task_succeed_done_cb)

    def _task_succeed_done_cb(self, future):
        try:
            ...
            self.get_logger().error(f'task succeed service call failed: {e}')
    ...
    def call_task_failed(self):
        ...
        future.add_done_callback(self._task_failed_done_cb)

    def _task_failed_done_cb(self, future):
        try:
            response = future.result()
            self.goal_pose_x = response.pose_x
            self.goal_pose_y = response.pose_y
            ...
            self.get_logger().error(f'task failed service call failed: {e}')
    ...
    def calculate_state(self):
        ...
        # 下采样 front_ranges 到 24 个值，以匹配 DQN agent 的 state_size=26
        # (26 - 2 = 24 个 LiDAR 射线)
        num_lidar_bins = 24
        if len(self.front_ranges) > 0:
            front_array = numpy.array(self.front_ranges)
            # 均匀采样：将 front_ranges 分成 num_lidar_bins 个桶，取每个桶的最小值
