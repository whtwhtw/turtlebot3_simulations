...
class Turtlebot3InteractiveMarker(Node):
    ...
    def publish_cmd_vel(self):
        twist = CmdVelMsg()
        twist_inner = _Twist() if ros_distro != 'humble' else twist
        ...
        if self.goal_position is not None:
            ...
            if distance < 0.01:
                ...
                twist_inner.linear.x = max(-0.1, min(0.1, forward_speed))
                twist_inner.linear.y = max(-0.1, min(0.1, lateral_speed))
            ...
            if abs(yaw_diff) < 0.01:
                ...
                twist_inner.angular.z = max(-0.5, min(0.5, yaw_diff))
